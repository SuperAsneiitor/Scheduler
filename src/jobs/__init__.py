...
- **可选发现**：:func:`discover_jobs` 遍历 ``jobs`` 子包并 ``import_module``，触发各模块中的类定义，
  从而完成注册（适合按目录组织的插件）。
"""
...
def discover_jobs(package_name: str = "jobs") -> None:
    """遍历 ``jobs`` 包（及子包）并导入所有模块，触发 ``BaseEDAJob`` 子类注册。

    Args:
        package_name: 通常为 ``jobs``，对应 ``src/jobs``。
    """
