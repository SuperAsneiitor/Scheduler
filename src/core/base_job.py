"""EDA 作业抽象基类：配合 :class:`jobs.JobRegistry` 做插件自注册。

设计说明：
- **为何使用 ``__init_subclass__``**：在子类 *定义完成* 时即可登记，无需手动维护
  ``setup.py`` entry points 或中央 ``if/elif`` 列表；导入模块即完成注册。
- **为何延迟导入 ``JobRegistry``**：避免 ``jobs`` 包与 ``core.base_job`` 之间的循环依赖。
"""
...
class BaseEDAJob(ABC):
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        # 延迟导入：保证 ``jobs.JobRegistry`` 在子类定义时已可用，且不形成顶层环依赖。
        from jobs import JobRegistry  # noqa: PLC0415
