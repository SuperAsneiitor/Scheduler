...
from executors.base import ExecutorJobState
from executors.exceptions import JobNotFoundError, LocalProcessStartError
...
class LocalExecutor:
    ...
    与 :class:`~executors.lsf_executor.LSFExecutor` 不同，本类**不**继承
    :class:`~executors.base.BaseExecutor`：接口为异步 ``submit_job(command, log_file)``，
    面向 ``asyncio`` 调度循环。
